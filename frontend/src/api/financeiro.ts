import type { PontoEquilibrio, ResultadoFinanceiro, LinhaRelatorioEquilibrio } from "../types/financeiro";
...
export async function fetchRelatorioPontoEquilibrio(
  lojaFiltro: number | "todas" | "minhas"
): Promise<LinhaRelatorioEquilibrio[]> {
  const params = new URLSearchParams({ lojaId: String(lojaFiltro) });
  const res = await fetch(`${API_BASE}/api/financeiro/ponto-equilibrio/relatorio?${params}`, { credentials: "include" });
  if (!res.ok) {
    const data = await res.json().catch(() => null);
    throw new Error(data?.error ?? `Erro ${res.status}`);
  }
