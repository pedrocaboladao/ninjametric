...
export async function importarRoteiro(
  ...
  for (const linhaBruta of texto.split(/\r?\n/)) {
    ...
    if (col.length >= 4 && Number.isFinite(pct) && col[2]) {
      ...
      let achadoSub = achado.id === null ? casar(nome, subs.rows) : { id: null, ambiguo: false };
      // Na tabela de cores o componente se chama "BASE ESPECIAL A" e so o
      // codigo da coluna B ("BASE A") casa com a formula cadastrada.
      if (achado.id === null && achadoSub.id === null && codigo) {
        const subPorCodigo = casar(codigo, subs.rows);
        if (subPorCodigo.id !== null) achadoSub = subPorCodigo;
      }
...
// --- importar varias formulas de uma vez -------------------------------------
//
// A tabela de cores tem as 15 cores numa aba so, cada bloco aberto pelo nome da
// cor. Importar uma por uma seria abrir a tela 15 vezes; aqui ele cola a aba
// inteira e cada bloco cai na sua formula.
//
// Cabecalho de tabela, TOTAL e o peso do lote sao ruido da planilha e caem
// fora — se virassem instrucao, a folha do chao de fabrica sairia com
// "ITEM CODIGO COMPONENTE" impresso no meio dos passos.

const RUIDO = [/^ITEM$/, /^TOTAL$/, /^PESO TOTAL$/, /^CORRECAO$/, /^OBS$/];

function linhaDeRuido(col: string[]): boolean {
  const preenchidas = col.filter((c) => c);
  if (!preenchidas.length) return true;
  if (RUIDO.some((re) => re.test(normalizar(preenchidas[0])))) return true;
  // "peso total: | 1400" aparece numa coluna qualquer da direita
  return preenchidas.some((c) => /^PESO TOTAL$/.test(normalizar(c)));
}
...
export async function importarLote(texto: string): Promise<ResultadoLote[]> {
  const { rows: formulas } = await pool.query<Achado>("SELECT id, nome FROM formulas");

  const blocos: { titulo: string; id: number; linhas: string[] }[] = [];
  for (const linhaBruta of texto.split(/\r?\n/)) {
    const linha = linhaBruta.replace(/\t+$/, "");
    if (!linha.trim()) continue;
    const col = linha.split("\t").map((c) => c.trim());
    if (linhaDeRuido(col)) continue;

    const preenchidas = col.filter((c) => c);
    if (preenchidas.length === 1) {
      // O titulo casa por igualdade, nao por "contem": uma instrucao como
      // "aguardar a Base C esfriar" viraria titulo e partiria o bloco no meio.
      const alvo = normalizar(preenchidas[0]);
      const iguais = formulas.filter((f) => normalizar(f.nome) === alvo);
      if (iguais.length === 1) {
        blocos.push({ titulo: preenchidas[0], id: iguais[0].id, linhas: [] });
        continue;
      }
    }
    // linha antes do primeiro titulo nao pertence a formula nenhuma
    if (blocos.length) blocos[blocos.length - 1].linhas.push(linha);
  }

  if (!blocos.length) {
    throw new Error(
      "Nenhum nome de fórmula reconhecido. Cada bloco precisa começar numa linha só com o nome da fórmula, escrito igual ao cadastro."
    );
  }

  const saida: ResultadoLote[] = [];
  for (const bloco of blocos) {
    try {
      saida.push({
        ...
        titulo: bloco.titulo,
        erro: null,
        resultado: await importarRoteiro(bloco.id, bloco.linhas.join("\n"), ""),
      });
    } catch (err) {
      saida.push({
        formulaId: bloco.id,
        titulo: bloco.titulo,
        erro: err instanceof Error ? err.message : "Falha ao importar.",
        resultado: null,
      });
    }
  }
